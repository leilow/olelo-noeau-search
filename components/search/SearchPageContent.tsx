...
export interface SearchPageContentProps {
  /** When provided (server-rendered), first paint has data and no client fetch. */
  initialAllPhrases?: Phrase[];
}

function SearchPageContentInner({ initialAllPhrases }: SearchPageContentProps) {
  const searchParams = useSearchParams();
  const router = useRouter();

  const [phrases, setPhrases] = useState<Phrase[]>([]);
  const [searchResults, setSearchResults] = useState<ReturnType<typeof searchPhrases>>([]);
  const [allPhrases, setAllPhrases] = useState<Phrase[]>(initialAllPhrases ?? []);
  const [loading, setLoading] = useState(initialAllPhrases === undefined);
  ...
  // When we have server-provided data, derive phraseNumberRange once (allPhrases/loading already from initial state)
  useEffect(() => {
    if (!initialAllPhrases?.length) return;
    let min = Infinity;
    let max = -Infinity;
    initialAllPhrases.forEach((p) => {
      if (p.phrase_numbers < min) min = p.phrase_numbers;
      if (p.phrase_numbers > max) max = p.phrase_numbers;
    });
    if (min !== Infinity && max !== -Infinity) {
      setPhraseNumberRange({ min, max });
    }
  }, [initialAllPhrases]);

  // Fetch all phrases only when not provided by server
  useEffect(() => {
    if (initialAllPhrases !== undefined) return;
    let cancelled = false;
    async function fetchPhrases() {
      try {
        ...
        data.forEach((p: Phrase) => {
          ...
          if (p.tags) {
            p.tags.forEach((tag) => {
              ...
              if (tagCategory) cats.add(tagCategory);
            });
          }
        });
        ...
        data.forEach((p: Phrase) => {
          if (p.hawaiian_letter) hawaiianLetterSet.add(p.hawaiian_letter.toLowerCase());
        });
  ...
  useEffect(() => {
    ...
    if (hawaiianLetters.length > 0) {
      filtered = filtered.filter(
        (p) =>
          p.hawaiian_letter &&
          hawaiianLetters.includes(p.hawaiian_letter.toLowerCase())
      );
    }
      ...
      filtered = filtered.filter((p) => {
        ...
        if (categories.length > 0) {
          ...
          tagsToCheck = p.tags.filter((tag) => {
            ...
            const normalizedTagCategory = tagCategory
              .split(' ')
              .map((word) => word.charAt(0).toLowerCase() + word.slice(1).toLowerCase())
              .join(' ');
    ...
    if (tags.length > 0) {
      filtered = filtered.filter(
        (p) => p.tags && tags.every((tag) => p.tags!.includes(tag))
      );
    }
    ...
    if (debouncedSearchQuery.length >= 2) {
      ...
      resultsWithMetadata = filtered.map((p) => ({
        ...p,
        relevanceScore: 0,
        matchedFields: [],
      }));
    ...
    if (tags.length > 0) {
      tags.forEach((tag) => {
        const tagCategory = tagMap[tag];
        if (tagCategory) selectedTagCategories.add(tagCategory);
      });
    }
    ...
    filtered.forEach((p: Phrase) => {
      ...
      if (p.tags) {
        p.tags.forEach((tag) => {
          tagSet.add(tag);
          const firstLetter = tag.charAt(0).toLowerCase();
          if (/[a-z]/.test(firstLetter)) indexLetterSet.add(firstLetter);
          const tagCategory = tagMap[tag];
          if (tagCategory) {
            ...
            if (shouldInclude) cats.add(tagCategory);
          }
        });
      }
    });
  ...
  useEffect(() => {
    ...
    if (phraseNumber.trim().length > 0)
      params.set('phraseNumber', phraseNumber.trim());
    ...
    <div className="container mx-auto px-4 sm:px-6 py-4 sm:py-8">
      <div className="space-y-6">
        ...
        <div id="results-start" className="text-sm text-text/70 font-mono text-center">
          {debouncedSearchQuery ? (
            <>
              Showing {phrases.length} of {filteredCount} results for
              &quot;{debouncedSearchQuery}&quot;
              {filteredCount < allPhrases.length &&
                ` (of ${allPhrases.length} total)`}
            </>
          ) : (
